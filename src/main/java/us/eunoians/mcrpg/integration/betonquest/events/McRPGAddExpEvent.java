...
public class McRPGAddExpEvent extends QuestEvent {

    private final McRPG plugin;
    private final NamespacedKey skillKey;
    private final VariableNumber experience;

    public McRPGAddExpEvent(@NotNull Instruction instruction) throws InstructionParseException {
    ...
    protected Void execute(@NotNull Profile profile) throws QuestRuntimeException {
        ...
        if (playerOptional.isPresent() && playerOptional.get() instanceof McRPGPlayer mcRPGPlayer) {
            ...
            if (skillData.isPresent()) {
                skillData.get().addExperience(experience.getValue(profile).intValue());
            }
        }
